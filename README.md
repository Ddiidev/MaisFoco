...
cat > .env << 'EOF'
HOST_DB=postgres
NAME_DB=MFMADATABASE
PASS_DB=2&bZg!0j0&5Bw@kY99*X*0O&!03nZE
USER_DB=postgres
EOF
```
...
RUN v install https://github.com/Ddiidev/recaptcha
...
Crie o arquivo `.env.local` na raiz do projeto com as variáveis de ambiente necessárias conforme a estrutura vista no arquivo conf_env.v:

```
NAME_DB=MFMADATABASE
NAME_DB_TEST=MFMADATABASE_TEST
HOST_DB=postgres
PORT_DB=5432
USER_DB=postgres
PASS_DB=2&bZg!0j0&5Bw@kY99*X*0O&!03nZE
PATH_PAGES=/app/path/to/pages
HANDLE_EMAIL_FROM=support@maisfoco.life
HANDLE_EMAIL_PORT=587
...
EMAIL_MAISFOCO_SUPORTE=suporte@maisfoco.life
RECAPTCHA=your_recaptcha_key
RECAPTCHA_SECRETKEY=your_recaptcha_secret
```
...
Este guia fornece uma configuração completa para executar o MaisFoco em containers Docker com alta disponibilidade e balanceamento de carga. Você pode ajustar as configurações conforme necessário para atender às suas necessidades específicas.
